...
// 内核不再自带内容：注入内容池（与 app.ux 装配的是同一份结构）
const CONTENT = await import(pathToFileURL(path.join(TMP, 'core', 'content', 'index.js')).href)
ENG.setContent(CONTENT.buildContent())

function makeRun(years, pathKind) {
  const run = ENG.newRun(20260912)
  const c = ENG.drawTalents(run, Math.min(3, ENG.counts().talents))
  ENG.chooseTalents(run, c.map(x => x.id))
  ENG.alloc(run, { iq: 6, eq: 5, hp: 6, luck: 3 })
  if (pathKind) { run.flags.dualPath = true; ENG.enterPath(run, pathKind) }
  for (let i = 0; i < years && run.alive; i++) ENG.step(run)
  return run
}

/** 渲染后的额外调整 */
const HOOKS = {
  life: vm => { if (vm.showInfo) vm.showInfo() }
}

const PRE_HOOKS = {
  index: app => { app.run = null },
  alloc: app => { app.run = makeRun(0) },
  life: app => { app.run = makeRun(30, 'body'); app.run.items = app.run.items.concat(['I001','I002']) },
  end: app => { app.run = makeRun(400, 'magic') },
  codex: app => {
    app.codexSeen = { T: ['T001', 'T002', 'T003', 'T004'], I: ['I001', 'I002'], D: ['D001'], A: ['A001', 'A002'] }
  },
  more: app => { app.codexSeen = { T: ['T001'], I: [], D: [], A: [] } }
}
